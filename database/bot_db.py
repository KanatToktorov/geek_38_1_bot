...
class Database:
    ...
    def sql_insert_profile(self, tg_id, nickname, bio, age, zodiac_sign, marital_status, address, photo):
        self.cursor.execute(
            sql_queries.INSERT_PROFILE_QUERY,
            (None, tg_id, nickname, bio, age, zodiac_sign, marital_status, address, photo)
        )
        self.connection.commit()
    ...
    def sql_select_all_profile(self, tg_id):
        self.cursor.row_factory = lambda cursor, row: {
            "id": row[0],
            "telegram_id": row[1],
            "nickname": row[2],
    ...
    def sql_select_all_referrals(self, tg_id):
        self.cursor.row_factory = lambda cursor, row: {
            "id": row[0],
            "owner_id": row[1],
            "telegram_id": row[2]
